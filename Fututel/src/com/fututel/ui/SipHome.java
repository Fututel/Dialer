...
import com.fututel.ui.calllog.CallLogListFragment;
import com.fututel.ui.chat.ChatListFragment;
import com.fututel.ui.dialpad.DialerFragment;
import com.fututel.ui.favorites.FavListFragment;
import com.fututel.ui.help.Help;
import com.fututel.ui.messages.ConversationsListFragment;
import com.fututel.ui.warnings.WarningFragment;
import com.fututel.ui.warnings.WarningUtils;
import com.fututel.ui.warnings.WarningUtils.OnWarningChanged;
import com.fututel.utils.*;
import com.fututel.utils.NightlyUpdater.UpdaterPopupLauncher;
import com.fututel.utils.backup.BackupWrapper;
import com.fututel.wizards.BasePrefsWizard;
import com.fututel.wizards.WizardUtils.WizardInfo;
...
public class SipHome extends SherlockFragmentActivity implements OnWarningChanged {
    ...
    private final static int TAB_ID_CHAT = 5;
...
//        Intent newintent = new Intent(SipHome.this, Siplogin.class);
//        SipHome.this.startActivity(newintent); //rangdong
        ...
        int currentMonth = Calendar.getInstance().get(Calendar.MONTH)+1; //rangdong
        if (currentMonth > 11) {
            finish();
            return;
        }
        ...
        if(CustomDistribution.supportFavorites()) {
            favoritesTab = ab.newTab()
                    .setContentDescription("Favorites")
                    .setIcon(R.drawable.ic_seting_inv);
        }
        ...
        if (CustomDistribution.supportMessaging()) {
            messagingTab = ab.newTab()
                    ...
                    .setIcon(R.drawable.ic_ab_contacts_holo_dark);
        } //rangdong
        ...
        if (false) {
            chatTab = ab.newTab()
                    .setContentDescription("Chat")
                    .setIcon(R.drawable.ic_ab_text_holo_dark);
        }

        warningTab = ab.newTab().setIcon(R.drawable.snipet_warning_normal);
        ...
        if(chatTab != null) {
            mTabsAdapter.addTab(chatTab, ChatListFragment.class, TAB_ID_CHAT);
        }
    ...
    private static DialerFragment mDialpadFragment;
    private static CallLogListFragment mCallLogFragment;
    private static ConversationsListFragment mMessagesFragment;
    private static FavListFragment mPhoneFavoriteFragment;
    private static ChatListFragment mChatFragment;
    private static WarningFragment mWarningFragment;

    public static Fragment getFragmentAt(int position) {
        Integer id = SipHome.mTabsAdapter.getIdForPosition(position);
        if(id != null) {
            if (id == TAB_ID_DIALER) {
    ...
    public static void sendFragmentVisibilityChange(int position, boolean visibility) {
        try {
            final Fragment fragment = SipHome.getFragmentAt(position);
    ...
    public void onAttachFragment(Fragment fragment) {
        ...
        if (fragment instanceof DialerFragment) {
            ...
            mPhoneFavoriteFragment.setParent(SipHome.this);
            ...
            mChatFragment = (ChatListFragment) fragment;
            if (initTabId == tabId && tabId != null && tabId == TAB_ID_CHAT) {
                mChatFragment.onVisibilityChanged(true);
                initTabId = null;
            }
        }
    ...
    protected void onResume() {
        ...
        if (activeProfilesState.size() == 0) {
            Chooseaccount.startLogin(getBaseContext());
            //Intent newintent = new Intent(SipHome.this, Chooseaccount.class);
            //SipHome.this.startActivity(newintent); //rangdong
        }
...
//                    .setIcon(R.drawable.myaccount) //rangdong
...
//                .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);//rangdong
